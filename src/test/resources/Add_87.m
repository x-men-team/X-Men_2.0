...
[ State($Human,'3',<$oyster,$ccard,bal($oyster),bal($ccard),~gid>)
, SndS($Human,$GateOut,<'balance','balance','gid'>,<bal($oyster),bal($ccard),~gid>)
]

rule GateOut_1_M:
[ State($GateOut,'1',<~goid>)
, RcvS($Human,$GateOut,<'balance','balance','gid'>,<bal($oyster),bal($ccard),~gid>)
]
--[ Receive($GateOut,$Human,$oyster)
, Receive($GateOut,$Human,bal($oyster))
...
[ State($GateOut,'2',<~goid,bal($oyster),bal($ccard),~gid>)
]
