...
[ State($Human,'2',<$oyster,$ccard,bal($oyster),bal($ccard)>)
, SndS($Human,$GateOut,<'card','card','balance'>,<$oyster,$ccard,bal($oyster)>)
]

rule GateOut_1_M:
[ State($GateOut,'1',<~goid>)
, RcvS($Human,$GateOut,<'card','card','balance'>,<$oyster,$ccard,bal($oyster)>)
]
--[ Receive($GateOut,$Human,$oyster)
, Receive($GateOut,$Human,bal($oyster))
...
[ State($GateOut,'2',<~goid,$oyster,$ccard,bal($oyster)>)
]
