...
[ State($Human,'3',<$oyster,$ccard,bal($oyster),bal($ccard),~gid>)
, SndS($Human,$GateOut,<'card','card','balance'>,<$oyster,$ccard,bal($ccard)>)
]

rule GateOut_1_M:
[ State($GateOut,'1',<~goid>)
, RcvS($Human,$GateOut,<'card','card','balance'>,<$oyster,$ccard,bal($ccard)>)
]
--[ Receive($GateOut,$Human,$oyster)
, Receive($GateOut,$Human,bal($oyster))
...
[ State($GateOut,'2',<~goid,$oyster,$ccard,bal($ccard)>)
]
