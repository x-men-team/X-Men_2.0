...
[ State($Human,'3',<$oyster,$ccard,bal($oyster),bal($ccard),~gid>)
, SndS($Human,$GateOut,<'balance','balance'>,<bal($oyster),bal($ccard)>)
]

rule GateOut_1_M:
[ State($GateOut,'1',<~goid>)
, RcvS($Human,$GateOut,<'balance','balance'>,<bal($oyster),bal($ccard)>)
]
--[ Receive($GateOut,$Human,$oyster)
, Receive($GateOut,$Human,bal($oyster))
...
[ State($GateOut,'2',<~goid,bal($oyster),bal($ccard)>)
]
