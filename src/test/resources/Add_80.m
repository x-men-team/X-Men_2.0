...
[ State($Human,'3',<$oyster,$ccard,bal($oyster),bal($ccard),~gid>)
, SndS($Human,$GateIn,<'card','card','balance','gid'>,<$oyster,$ccard,bal($ccard),~gid>)
]

rule GateIn_1_M:
[ State($GateIn,'1',<~gid>)
, RcvS($Human,$GateIn,<'card','card','balance','gid'>,<$oyster,$ccard,bal($ccard),~gid>)
]
--[ Receive($GateIn,$Human,$oyster)
...
[ State($GateIn,'2',<~gid,$oyster,$ccard,bal($ccard)>)
]
