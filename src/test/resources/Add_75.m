...
[ State($Human,'3',<$oyster,$ccard,bal($oyster),bal($ccard),~gid>)
, SndS($Human,$GateOut,<'card','balance','gid'>,<$ccard,bal($oyster),~gid>)
]

rule GateOut_1_M:
[ State($GateOut,'1',<~goid>)
, RcvS($Human,$GateOut,<'card','balance','gid'>,<$ccard,bal($oyster),~gid>)
]
--[ Receive($GateOut,$Human,$oyster)
, Receive($GateOut,$Human,bal($oyster))
...
[ State($GateOut,'2',<~goid,$ccard,bal($oyster),~gid>)
]
