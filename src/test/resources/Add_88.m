...
[ State($Human,'3',<$oyster,$ccard,bal($oyster),bal($ccard),~gid>)
, SndS($Human,$GateIn,<'card','card','balance','balance','gid'>,<$oyster,$ccard,bal($oyster),bal($ccard),~gid>)
]

rule GateIn_1_M:
[ State($GateIn,'1',<~gid>)
, RcvS($Human,$GateIn,<'card','card','balance','balance','gid'>,<$oyster,$ccard,bal($oyster),bal($ccard),~gid>)
]
--[ Receive($GateIn,$Human,$oyster)
, Rcv($GateIn,$Human,$oyster)
, Rcv($GateIn,$Human,$ccard)
, Rcv($GateIn,$Human,bal($oyster))
, Rcv($GateIn,$Human,bal($ccard))
, Rcv($GateIn,$Human,~gid)
]->
[ State($GateIn,'2',<~gid,$oyster,$ccard,bal($oyster),bal($ccard)>)
]
