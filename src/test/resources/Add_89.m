...
--[ Snd($Human,'card',$oyster)
, Snd($Human,'card',$ccard)
, Snd($Human,'balance',bal($oyster))
, Snd($Human,'balance',bal($ccard))
, Snd($Human,'gid',~gid)
]->
[ State($Human,'3',<$oyster,$ccard,bal($oyster),bal($ccard),~gid>)
, SndS($Human,$GateOut,<'card','card','balance','balance','gid'>,<$oyster,$ccard,bal($oyster),bal($ccard),~gid>)
]

rule GateOut_1_M:
[ State($GateOut,'1',<~goid>)
, RcvS($Human,$GateOut,<'card','card','balance','balance','gid'>,<$oyster,$ccard,bal($oyster),bal($ccard),~gid>)
]
--[ Receive($GateOut,$Human,$oyster)
, Receive($GateOut,$Human,bal($oyster))
, Rcv($GateOut,$Human,$oyster)
, Rcv($GateOut,$Human,$ccard)
, Rcv($GateOut,$Human,bal($oyster))
, Rcv($GateOut,$Human,bal($ccard))
, Rcv($GateOut,$Human,~gid)
]->
[ State($GateOut,'2',<~goid,$oyster,$ccard,bal($oyster),bal($ccard),~gid>)
]
